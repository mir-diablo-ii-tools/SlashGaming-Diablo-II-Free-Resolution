...
void DrawLeftScreenBackground() {
  ::d2::CelFile_Api& screen_background =
      GetCelFile(config::GetScreenBackgroundImagePath());
  ...
  for (unsigned int row = 0; height_covered < resolution.width; row += 1) {
    int width_covered = 0;
    unsigned int row_start_frame = (row % 2) * half_num_frames;

    for (unsigned int col = 0;
        width_covered < (resolution.width / 2) - 1;
        col += 1) {
      unsigned int current_frame =
          row_start_frame + ((half_num_frames - 1) - (col % half_num_frames));

      screen_background.DrawFrame(
          (resolution.width / 2)
              - width_covered
              + cels.at(current_frame).GetOffsetX(),
...
void DrawOriginalLeftScreenBorderFrame() {
  ::d2::CelFile_Api& screen_border_frame =
      GetCelFile(kOriginalScreenBorderFrameImagePath);

  Resolution resolution =
      GetIngameResolutionFromId(d2::d2gfx::GetResolutionMode());

  const int left = (resolution.width - 640 - (800 - 640)) / 2;
  const int top = ((resolution.height - 480 - (600 - 480)) / 2) - 3;
  ...
  screen_border_frame.DrawFrame(
      left + cels.at(0).GetWidth(), top, 0, 1, frame_options);
...
void DrawOriginalRightScreenBorderFrame() {
  ...
  const int right = (resolution.width + 640 + (800 - 640)) / 2;
  const int top = ((resolution.height - 480 - (600 - 480)) / 2) - 3;
  ...
  screen_border_frame.DrawFrame(
      right - cels.at(6).GetWidth(), top, 0, 5, frame_options);

  screen_border_frame.DrawFrame(
      right, top + cels.at(6).GetHeight(), 0, 7, frame_options);
...
void DrawCustomLeftScreenBorderFrame() {
  ::d2::CelFile_Api& screen_left_border =
      GetCelFile(config::GetCustomLeftScreenBorderLeftImagePath());
  ::d2::CelFile_Api& screen_top_border =
      GetCelFile(config::GetCustomLeftScreenBorderTopImagePath());
  ::d2::CelFile_Api& screen_top_right_border =
      GetCelFile(config::GetCustomLeftScreenBorderTopRightImagePath());
  ::d2::CelFile_Api& screen_bottom_border =
      GetCelFile(config::GetCustomLeftScreenBorderBottomImagePath());
  ::d2::CelFile_Api& screen_bottom_right_border =
      GetCelFile(config::GetCustomLeftScreenBorderBottomRightImagePath());

  Resolution resolution =
      GetIngameResolutionFromId(d2::d2gfx::GetResolutionMode());

  // Determine border starting positions.
  const int screen_left = (resolution.width - 640) / 2;
  const int screen_top = (resolution.height - 480) / 2;

  int screen_left_border_height = 0;

  for (unsigned int frame = 0;
      frame < screen_left_border.GetNumFrames();
      frame += 1) {
  ...
  const int border_top =
      screen_top + (((256 + 176) - screen_left_border_height) / 2);
  ...
  for (unsigned int frame = 0;
      frame < screen_top_right_border.GetNumFrames();
      frame += 1) {
  ...
  screen_top_right_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kRight;
  screen_top_right_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kTop;
  ...
  for (unsigned int frame = 0;
      frame < screen_top_border.GetNumFrames();
      frame += 1) {
  ...
  screen_top_border.DrawAllFrames(
      (resolution.width / 2) - screen_top_right_border_width,
      border_top,
  ...
  for (unsigned int frame = 0;
      frame < screen_bottom_right_border.GetNumFrames();
      frame += 1) {
  ...
  screen_bottom_border.DrawAllFrames(
      (resolution.width / 2) - screen_bottom_right_border_width,
      border_bottom,
...
void DrawCustomRightScreenBorderFrame() {
  ::d2::CelFile_Api& screen_right_border =
      GetCelFile(config::GetCustomRightScreenBorderRightImagePath());
  ::d2::CelFile_Api& screen_top_border =
      GetCelFile(config::GetCustomRightScreenBorderTopImagePath());
  ::d2::CelFile_Api& screen_top_left_border =
      GetCelFile(config::GetCustomRightScreenBorderTopLeftImagePath());
  ::d2::CelFile_Api& screen_bottom_border =
      GetCelFile(config::GetCustomRightScreenBorderBottomImagePath());
  ::d2::CelFile_Api& screen_bottom_left_border =
      GetCelFile(config::GetCustomRightScreenBorderBottomLeftImagePath());

  Resolution resolution =
      GetIngameResolutionFromId(d2::d2gfx::GetResolutionMode());

  // Determine border starting positions.
  const int screen_right = (resolution.width + 640) / 2;
  const int screen_top = (resolution.height - 480) / 2;

  int screen_right_border_height = 0;

  for (unsigned int frame = 0;
      frame < screen_right_border.GetNumFrames();
      frame += 1) {
  ...
  const int border_top =
      screen_top + (((256 + 176) - screen_right_border_height) / 2);
  const int border_bottom =
      screen_top + (((256 + 176) + screen_right_border_height) / 2);
  ...
  for (unsigned int frame = 0;
      frame < screen_top_left_border.GetNumFrames();
      frame += 1) {
  ...
  screen_top_left_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kLeft;
  screen_top_left_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kTop;
  ...
  screen_top_border.DrawAllFrames(
      (resolution.width / 2) + screen_top_left_border_width,
      border_top,
      screen_top_border.GetNumFrames(),
      1,
      screen_top_all_frame_options);
  ...
  for (unsigned int frame = 0;
      frame < screen_bottom_left_border.GetNumFrames();
      frame += 1) {
  ...
  screen_bottom_border.DrawAllFrames(
      (resolution.width / 2) + screen_bottom_left_border_width,
      border_bottom,
      screen_bottom_border.GetNumFrames(),
      1,
      screen_bottom_all_frame_options);
...
void DrawLeftScreenBackgroundRibbon() {
  ::d2::CelFile_Api& screen_border_horizontal_ribbon =
      GetCelFile(config::GetScreenBorderHorizontalRibbonImagePath());
  ::d2::CelFile_Api& screen_border_vertical_ribbon =
      GetCelFile(config::GetScreenBorderVerticalRibbonImagePath());
  ...
  const int screen_left = (resolution.width - 640) / 2;
  const int screen_right = resolution.width / 2;
  const int screen_top = (resolution.height - 480) / 2;
  ...
  const int border_top =
      screen_top + (((256 + 176) - screen_left_border_height) / 2);
  const int border_left =
      screen_left - ::d2::Cel_View(screen_left_border.GetCel(0, 0)).GetWidth();
  const int border_bottom =
      screen_top + (((256 + 176) + screen_left_border_height) / 2);

  // Draw horizontal bars.
  ::d2::DrawAllCelFileFramesOptions
      screen_top_horizontal_ribbon_all_frame_options;
  screen_top_horizontal_ribbon_all_frame_options.cel_file_direction = 0;
  screen_top_horizontal_ribbon_all_frame_options.color =
      ::mapi::Rgba32BitColor();
  screen_top_horizontal_ribbon_all_frame_options.draw_effect =
      ::d2::DrawEffect::kNone;
  screen_top_horizontal_ribbon_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kRight;
  screen_top_horizontal_ribbon_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kTop;

  ::d2::DrawAllCelFileFramesOptions
      screen_bottom_horizontal_ribbon_all_frame_options;
  screen_bottom_horizontal_ribbon_all_frame_options.cel_file_direction = 0;
  screen_bottom_horizontal_ribbon_all_frame_options.color =
      ::mapi::Rgba32BitColor();
  screen_bottom_horizontal_ribbon_all_frame_options.draw_effect =
      ::d2::DrawEffect::kNone;
  screen_bottom_horizontal_ribbon_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kRight;
  screen_bottom_horizontal_ribbon_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kBottom;

  int screen_border_horizontal_ribbon_width = 0;
  for (unsigned int frame = 0;
      frame < screen_border_horizontal_ribbon.GetNumFrames();
      frame += 1) {
  ...
  screen_top_left_vertical_ribbon_all_frame_options.color =
      ::mapi::Rgba32BitColor();
  screen_top_left_vertical_ribbon_all_frame_options.draw_effect =
      ::d2::DrawEffect::kNone;
  screen_top_left_vertical_ribbon_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kLeft;
  screen_top_left_vertical_ribbon_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kBottom;
  ...
  screen_top_right_vertical_ribbon_all_frame_options.color =
      ::mapi::Rgba32BitColor();
  screen_top_right_vertical_ribbon_all_frame_options.draw_effect =
      ::d2::DrawEffect::kNone;
  screen_top_right_vertical_ribbon_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kRight;
  screen_top_right_vertical_ribbon_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kBottom;

  int screen_border_vertical_ribbon_height = 0;
  for (unsigned int frame = 0;
      frame < screen_border_vertical_ribbon.GetNumFrames();
      frame += 1) {
  ...
  screen_bottom_left_vertical_ribbon_all_frame_options.color =
      ::mapi::Rgba32BitColor();
  screen_bottom_left_vertical_ribbon_all_frame_options.draw_effect =
      ::d2::DrawEffect::kNone;
  screen_bottom_left_vertical_ribbon_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kLeft;
  screen_bottom_left_vertical_ribbon_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kTop;
  ...
  screen_bottom_right_vertical_ribbon_all_frame_options.color =
      ::mapi::Rgba32BitColor();
  screen_bottom_right_vertical_ribbon_all_frame_options.draw_effect =
      ::d2::DrawEffect::kNone;
  screen_bottom_right_vertical_ribbon_all_frame_options.position_x_behavior =
      ::d2::DrawPositionXBehavior::kRight;
  screen_bottom_right_vertical_ribbon_all_frame_options.position_y_behavior =
      ::d2::DrawPositionYBehavior::kTop;
...
void DrawRightScreenBackgroundRibbon() {
  ...
  const int screen_left = resolution.width / 2;
  const int screen_right = (resolution.width + 640) / 2;
  const int screen_top = (resolution.height - 480) / 2;
  ...
  const int border_right =
      screen_right + d2::Cel_View(screen_right_border.GetCel(0, 0)).GetWidth();
  const int border_bottom =
      border_top + screen_right_border_height;
  ...
  while (width_covered < (resolution.width - border_right)) {
    screen_border_horizontal_ribbon.DrawAllFrames(
